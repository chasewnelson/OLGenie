...
# EXAMPLES FOR LINE-BY-LINE DEBUGGING
#CODON_RESULTS_FILE <- "~/Desktop/SARS-CoV-2/within_Sarbecovirus_n21/SARBECOVIRUS_functional_subsets/OLGenie/ORF3a/ss12_wP5L/OLGenie_codon_results.txt"
#CODON_RESULTS_FILE <- "~/Desktop/SCIENCE/OLGenie/Oscar_example.txt"
...
dNdS_diff_boot_fun <- function(codon_results, numerator, denominator, num_replicates, num_cpus) {
 ...
 (dN <- sum(as.numeric(unlist(codon_results[ , paste0(numerator, "_diffs")])), na.rm = TRUE) / sum(as.numeric(unlist(codon_results[ , paste0(numerator, "_sites")])), na.rm = TRUE))
 (dS <- sum(as.numeric(unlist(codon_results[ , paste0(denominator, "_diffs")])), na.rm = TRUE) / sum(as.numeric(unlist(codon_results[ , paste0(denominator, "_sites")])), na.rm = TRUE))
 (dNdS <- dN / dS)
...
summary_data <- paste(CODON_RESULTS_FILE,
                      nrow(codon_results),
...
#[15] ASL_dN_gt_dS_P
#[16] ASL_dN_lt_dS_P
header <- c('file', 'num_codons', 'NN_sites', 'SN_sites', 'NS_sites', 'SS_sites', 'NN_diffs', 'SN_diffs', 'NS_diffs', 'SS_diffs', 
