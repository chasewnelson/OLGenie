...
### overlapping gene selection simulation, frame sas11

############################################################################################################
### EXAMPLE:
# ./simulation_sas11.pl --dnds1=0.1 --dnds2=0.1 --num_codons=100 --num_seqs=100 --distance=0.05 --R=1 --outfile=sim_output.fasta
############################################################################################################
...
if($distance_actual =~ /[0-9]/ && $distance_actual ne 'NA') {
	...
	#print "distances_expected=" . $distances_expected{'sas11'}->{0.5}->{2}->{2} . "\n";
	...
	my $correction_factor = $distance_actual / $distances_expected{'sas11'}->{$R_for_correction}->{$dnds1_for_correction}->{$dnds2_for_correction};
...
###--------------------------main part--------------------------------------
        #open(my $fh, '>', "SAS11\_PhylogenyDistance_$distance\_dnds1_$dnds_1\_dnds2_$dnds_2\_R_$R.txt");
        open(my $fh, '>', "$outfile");
