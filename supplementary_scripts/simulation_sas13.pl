...
### overlapping gene selection simulation, frame sas13

############################################################################################################
### EXAMPLE:
# ./simulation_sas13.pl --dnds1=0.1 --dnds2=0.1 --num_codons=100 --num_seqs=100 --distance=0.05 --R=1 --outfile=sim_output.fasta
############################################################################################################

####sense antisense 123
####################321
############select codon, produce antisense sequences, introduce mutation according to codon number,
###calculate pathway codon by codon 
use strict;   
use warnings; 
use diagnostics;
use autodie;
...
if($distance_actual =~ /[0-9]/ && $distance_actual ne 'NA') {
	...
	#print "distances_expected=" . $distances_expected{'sas13'}->{0.5}->{2}->{2} . "\n";
	...
	my $correction_factor = $distance_actual / $distances_expected{'sas13'}->{$R_for_correction}->{$dnds1_for_correction}->{$dnds2_for_correction};
...
# Correct distance to obtain the actual distance outcome of the biological controls (0.0685)
# The correction factor was empirically determined for $distance == 0.05
#if($distance == 0.05) {
#	if($dnds_1_values[0] == 0.1 && $dnds_2_values[0] == 0.1) {
#		$distance = (14.95821218 * $distance);
#	} elsif(($dnds_1_values[0] == 0.1 && $dnds_2_values[0] == 0.5) || ($dnds_1_values[0] == 0.5 && $dnds_2_values[0] == 0.1)) {
#		$distance = (7.460019977 * $distance);
#	} elsif(($dnds_1_values[0] == 0.1 && $dnds_2_values[0] == 1) || ($dnds_1_values[0] == 1 && $dnds_2_values[0] == 0.1)) {
#		$distance = (4.625380734 * $distance);
#	} elsif(($dnds_1_values[0] == 0.1 && $dnds_2_values[0] == 1.5) || ($dnds_1_values[0] == 1.5 && $dnds_2_values[0] == 0.1)) {
#		$distance = (3.367878666 * $distance);
#	} elsif(($dnds_1_values[0] == 0.1 && $dnds_2_values[0] == 2) || ($dnds_1_values[0] == 2 && $dnds_2_values[0] == 0.1)) {
#		$distance = (2.671661981 * $distance);
#	} elsif($dnds_1_values[0] == 0.5 && $dnds_2_values[0] == 0.5) {
#		$distance = (3.85213624 * $distance);
#	} elsif(($dnds_1_values[0] == 0.5 && $dnds_2_values[0] == 1) || ($dnds_1_values[0] == 1 && $dnds_2_values[0] == 0.5)) {
#		$distance = (2.428619523 * $distance);
#	} elsif(($dnds_1_values[0] == 0.5 && $dnds_2_values[0] == 1.5) || ($dnds_1_values[0] == 1.5 && $dnds_2_values[0] == 0.5)) {
#		$distance = (1.782092438 * $distance);
#	} elsif(($dnds_1_values[0] == 0.5 && $dnds_2_values[0] == 2) || ($dnds_1_values[0] == 2 && $dnds_2_values[0] == 0.5)) {
#		$distance = (1.413779172 * $distance);
#	} elsif(($dnds_1_values[0] == 0.5 && $dnds_2_values[0] == 2) || ($dnds_1_values[0] == 2 && $dnds_2_values[0] == 0.5)) {
#		$distance = (1.413779172 * $distance);
#	} elsif($dnds_1_values[0] == 1 && $dnds_2_values[0] == 1) {
#		$distance = (1.538370646 * $distance);
#	} elsif(($dnds_1_values[0] == 1 && $dnds_2_values[0] == 1.5) || ($dnds_1_values[0] == 1.5 && $dnds_2_values[0] == 1)) {
#		$distance = (1.136880148 * $distance);
#	} elsif(($dnds_1_values[0] == 1 && $dnds_2_values[0] == 2) || ($dnds_1_values[0] == 2 && $dnds_2_values[0] == 1)) {
#		$distance = (0.908813382 * $distance);
#	} elsif($dnds_1_values[0] == 1.5 && $dnds_2_values[0] == 1.5) {
#		$distance = (0.846135762 * $distance);
#	} elsif(($dnds_1_values[0] == 1.5 && $dnds_2_values[0] == 2) || ($dnds_1_values[0] == 2 && $dnds_2_values[0] == 1.5)) {
#		$distance = (0.679903656 * $distance);
#	} elsif($dnds_1_values[0] == 2 && $dnds_2_values[0] == 2) {
#		$distance = (0.54922968 * $distance);
#	} else {
#		print "### WARNING: the distance will only be corrected to yield 0.0685 for distance=0.5 and dN/dS estimates of 0.1, 0.5, 1, 1.5, or 2\n";
#	}
#} else {
#	print "### WARNING: the distance will only be corrected to yield 0.0685 for distance=0.5 and dN/dS estimates of 0.1, 0.5, 1, 1.5, or 2\n";
#}
...
###--------------------------main part--------------------------------------
		#open(my $fh, '>', "SAS13\_PhylogenyDistance\_$distance\_dnds1_$dnds_1\_dnds2_$dnds_2\_R_$R.txt");
		open(my $fh, '>', "$outfile");
...
sub selection{  ###
	...
	if ($position%3==2){
		...
		$codon_original_2=reverse($codon_original_1);
		$codon_original_2=~tr/ACGTacgt/TGCAtgca/;### 
		$codon_mutation_1=$codon_original_1;  
		substr($codon_mutation_1,2,1,$newbase); 
		$codon_mutation_2=reverse($codon_mutation_1);
		$codon_mutation_2=~tr/ACGTacgt/TGCAtgca/;
	} elsif($position%3==0){
...
sub random_codon{####
	my(@codons)=('TCC','TCG','TCT','TTC','TTT','TTG','TAC','TAT','TGC','TGT','TGG','CTC','CTG','CTT','CCA','CCC','CCG','CCT','CAC','CAT','CAA','CAG','CGA','CGC','CGG','CGT','ATA','ATC','ATT','ATG','ACA','ACC','ACG','ACT','AAC','AAT','AAA','AAG','AGC','AGT','AGA','AGG','GTA','GTC','GTG','GTT','GCA','GCC','GCG','GCT','GAC','GAT','GAA','GAG','GGA','GGC','GGG','GGT');
